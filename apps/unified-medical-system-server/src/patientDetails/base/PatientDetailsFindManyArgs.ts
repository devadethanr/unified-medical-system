/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { PatientDetailsWhereInput } from "./PatientDetailsWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { PatientDetailsOrderByInput } from "./PatientDetailsOrderByInput";

@ArgsType()
class PatientDetailsFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => PatientDetailsWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => PatientDetailsWhereInput, { nullable: true })
  @Type(() => PatientDetailsWhereInput)
  where?: PatientDetailsWhereInput;

  @ApiProperty({
    required: false,
    type: [PatientDetailsOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [PatientDetailsOrderByInput], { nullable: true })
  @Type(() => PatientDetailsOrderByInput)
  orderBy?: Array<PatientDetailsOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { PatientDetailsFindManyArgs as PatientDetailsFindManyArgs };
